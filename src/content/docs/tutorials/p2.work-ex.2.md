...
description: 频率响应 worked examples 与 GPT 生成的参考答案。
---

![anny2](../../../assets/anny2.jpg)
...
- **a.** $G(j\omega)=\frac{10}{(1+j\omega)^2}$ — **稳定**。$1+G=0\Rightarrow s^2+2s+11=0$，二阶系数全正。Nyquist 不包围 $-1$。
- **b.** $G(j\omega)=\frac{8}{j\omega(1+j\omega/5)(1+j\omega/10)}$ — **稳定**。$1+G=0\Rightarrow s^3+15s^2+50s+400=0$，Routh 第一列为 $1,15,70/3,400$。相位交越满足 $\omega_{pc}=\sqrt{50}$，$\lvert G(j\omega_{pc})\rvert=8/15$，所以 $GM=15/8\gt1$。
- **c.** $G(j\omega)=\frac{10}{(j\omega)^2(1+j\omega/10)}$ — **不稳定**。$1+G=0\Rightarrow s^3+10s^2+100=0$，Routh 第一列出现负项，闭环有右半平面极点。
- **d.** $G(j\omega)=\frac{10(1+j\omega/2)}{(j\omega)^2(1+j\omega/10)}$ — **稳定**。$1+G=0\Rightarrow s^3+10s^2+50s+100=0$，Routh 第一列为 $1,10,40,100$。零点提供相位超前，Nyquist 避开 $-1$。
- **e.** $G(j\omega)=\frac{4(1+j\omega)^2}{(j\omega)^2(1+5j\omega)}$ — **稳定；但增益除以 $3$ 后不稳定**。一般令分子增益为 $K$，特征方程为 $5s^3+(1+K)s^2+2Ks+K=0$。Routh 条件给 $K\gt1.5$。原题 $K=4$ 稳定；除以 $3$ 后 $K=4/3\lt1.5$，不稳定。
