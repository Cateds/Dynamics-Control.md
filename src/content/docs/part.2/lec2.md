...
这一讲开头先接着 [Lec.1](./lec1.md) 的根轨迹做了一个 guided classwork。
...
频率响应分析的意义在于：它可以告诉我们系统在不同频率输入下的表现。对于控制系统来说，这和稳定性、鲁棒性、带宽都有关系。
...
- 频率响应描述系统对不同频率正弦输入的稳态响应
- $G(j\omega)$ 的幅值给出增益，相角给出相位变化
- Nyquist 图是 $G(j\omega)$ 在复平面上的轨迹
- 积分、lag、lead 等基本环节有典型的轨迹形状
- 后面判断稳定性时，关键会变成 Nyquist 图和 $-1+j0$ 的关系
